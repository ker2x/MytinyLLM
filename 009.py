# After a few hours of training on my poor macbook Air
# === Multi-Level Evaluation ===
# Evaluating 1-digit problems (500 samples)... 500/500 = 100.00%
#   +: 103/103 = 100.00%
#   -: 126/126 = 100.00%
#   *: 144/144 = 100.00%
#   /: 127/127 = 100.00%
# Evaluating 2-digit problems (500 samples)... 484/500 = 96.80%
#  +: 111/111 = 100.00%
#  -: 119/125 = 95.20%
#  *: 135/137 = 98.54%
#  /: 119/127 = 93.70%
# Evaluating 3-digit problems (500 samples)... 386/500 = 77.20%
#   +: 117/117 = 100.00%
#   -: 106/117 = 90.60%
#   *: 35/129 = 27.13%
#   /: 128/137 = 93.43%
# Extrapolation: 4-digit problems (50 samples)... 9/50 = 18.00%
#   +: 0/10 = 0.00%
#   -: 0/10 = 0.00%
#   *: 0/14 = 0.00%
#   /: 9/16 = 56.25%
# Difficulty   | Peak (Training)      | Final Eval      | Change
# ----------------------------------------------------------------------
# 1-digit       | Stage 1: 100.0%         |  100.0%        | +0.0%
# 2-digit       | Stage 2: 100.0%         |   96.8%        | -3.2%
# 3-digit       | Stage 2:  85.0%         |   77.2%        | -7.8%

"""
009.py — Tiny arithmetic LLM with Scratchpad Training
...
def _make_div_pair(max_digits: int) -> Tuple[int, int]:
    ...
    # Simply generate two random numbers within the digit constraint
    a = _rand_int(max_digits)  # dividend
    b = random.randint(1, 10 ** max_digits - 1)  # divisor (non-zero)
...
def generate_scratchpad_pair(cfg: GenConfig) -> Tuple[str, str | None]:
    ...
    and improve generalization. Division only returns one sample since it's
    not commutative.
    ...
    if op == '+':
        ...
        # Don't generate reversed for division
        t2 = None
        return t1, t2
    else:
        raise ValueError(f"Unknown operator: {op}")
...
def make_batch(tokenizer: CharTokenizer, batch_size: int, cfg: GenConfig, device: torch.device, max_pos: int) -> Tuple[
    ...
    (except division, which only generates A/B). Pads sequences to uniform length
    and creates shifted input/target pairs for language modeling.
