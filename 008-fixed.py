"""
008.py — Tiny arithmetic LLM with Scratchpad Training
...
def _make_div_pair(max_digits: int) -> Tuple[int, int]:
    """Generate operand pair (a, b) for integer division (remainder discarded).

    Generates two random numbers within max_digits constraint.
    Division is integer division (//), remainder is ignored.
...
def generate_scratchpad_pair(cfg: GenConfig) -> tuple[str, str | None]:
    ...
    Returns (A op B, B op A). For division, returns only (A/B, None).
    """
    op = _choose_op(cfg)
    if op == '+':
        a, b = _rand_int(cfg.max_digits), _rand_int(cfg.max_digits)
...
def make_batch(tokenizer: CharTokenizer, batch_size: int, cfg: GenConfig, device: torch.device, max_pos: int) -> Tuple[
    ...
    (except division, which only generates A/B).
...
def main():
    ...
    p_demo.add_argument('--ckpt', type=str, default='math_llm_scratchpad_model-008.pt',
                        help="Path to checkpoint file to load trained model from (default: math_llm_scratchpad_model-008.pt)")
