...
def train(
    ...
    if eval_samples and eval_samples > 0:
        ...
        for eval_digits in range(1, max_digits + 1):
            ...
            stats = evaluate(model, tokenizer, eval_cfg, n_samples=eval_samples, device=dev, max_pos=max_pos, verbose=False, save_outputs=save_path)
            overall = stats['overall']
            overall_acc = 100.0 * overall['correct'] / max(1, overall['total'])
        ...
        stats_4d = evaluate(model, tokenizer, eval_cfg_4d, n_samples=extrapolation_samples, device=dev, max_pos=max_pos, verbose=False, save_outputs=save_path_4d)
