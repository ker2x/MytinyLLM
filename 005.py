...
def generate_augmented_samples(cfg: GenConfig) -> List[str]:
    """Generate multiple training samples from a single (A,B) draw.

    For the drawn pair (A,B), produce training strings for every operator in
    cfg.ops and for both operand orders (A op B) and (B op A).

    Division samples are only included when the division is exact (integer
    result, no divide-by-zero) for that specific order, preserving the original
    guarantee of integer-only division examples.
    """
    a = _rand_int(cfg.max_digits)
    b = _rand_int(cfg.max_digits)
    # Ensure neither operand is zero; resample until both non-zero
    while a == 0 or b == 0:
        a = _rand_int(cfg.max_digits)
        b = _rand_int(cfg.max_digits)
    out: List[str] = []
    for op in cfg.ops:
        if op == '+':
            ...
            out.append(f"{b}+{a}={b + a}\n")
        elif op == '-':
            out.append(f"{a}-{b}={a - b}\n")
            out.append(f"{b}-{a}={b - a}\n")
        elif op == '*':
            prod = a * b
            out.append(f"{a}*{b}={prod}\n")
            out.append(f"{b}*{a}={prod}\n")
        elif op == '/':
            out.append(f"{a}/{b}={a // b}\n")
            out.append(f"{b}/{a}={b // a}\n")
        else:
...
def make_batch(tokenizer: CharTokenizer, batch_size: int, cfg: GenConfig, device: torch.device) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    Training is augmented: for each random (A,B) draw we generate multiple
    samples covering all operators in cfg.ops and both operand orders (A op B
    and B op A). Division examples are included only when exact integer division
    holds for that order.

    Sequences are padded to the maximum length in the batch. Inputs are all
    tokens except the last; targets are all tokens except the first (a standard
    next-token prediction setup). Returns tensors of shape (B, T) on the
    specified device.
    """
    samples: List[str] = []
    # Keep adding augmented groups until we have at least batch_size samples
    while len(samples) < batch_size:
        samples.extend(generate_augmented_samples(cfg))
...
def main():
    ...
    p_train.add_argument('--op-probs', type=str, default='0.03,0.07,0.60,0.30', help="Comma-separated probabilities aligned with --ops (default biases: +:0.03, -:0.07, *:0.60, /:0.30). Example override: '0.1,0.1,0.7,0.1'")
